    ...
    def refresh_data(time_period, include_tests):
        ...
        try:
            ...
            if 'error' in summary and summary['error']:
                print(f"Error in token dashboard: {summary['error']}")
                return (
            ...
            **Total Requests:** {total_requests}
            ...
            if not model_data:
                print("No model data for charts")
                return (
                    "No model usage data available for charts.",
                    None, None, None, None, None
                )
            ...
            if summary.get('daily_usage'):
                daily_data = summary.get('daily_usage', [])
                if daily_data:
                    daily_df = pd.DataFrame(daily_data)
                    if not daily_df.empty and 'date' in daily_df.columns and 'tokens' in daily_df.columns:
                        daily_fig = px.line(
                            daily_df, 
                            x="date", 
                            y="tokens",
            ...
            # If no daily data, create an empty chart
            if daily_fig is None:
                daily_fig = go.Figure()
                daily_fig.update_layout(
                    title="Daily Token Usage (No Data)", 
                    xaxis_title="Date",
                    yaxis_title="Total Tokens"
                )
            
            print(f"Successfully generated token dashboard charts for {time_period} days")
            ...
            empty_bar = go.Figure()
            empty_bar.update_layout(title="Token Usage (Error Loading Data)")
            
            empty_pie = go.Figure()
            empty_pie.update_layout(title="Template Usage (Error Loading Data)")
            
            empty_gauge = go.Figure()
            empty_gauge.update_layout(title="Fallback Rate (Error Loading Data)")
            
            empty_line = go.Figure()
            empty_line.update_layout(title="Daily Usage (Error Loading Data)")
            
            error_message = f"""
            ## Error Loading Token Analytics
            
            There was an error retrieving token usage data: {str(e)}
            
            Please try refreshing the data or contact support if the issue persists.
            """
            
            return error_message, empty_bar, empty_bar, empty_pie, empty_gauge, empty_line
