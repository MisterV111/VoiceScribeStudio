...
# Re-export load_voices function with a slightly different implementation
def load_voices():
    """Load voice options for the dropdown menus"""
    try:
        # Use the Eleven Labs client implementation
        voices = get_voices()
        
        # Extract preset voices based on the voice response
        # Format as needed for the UI components
        if not voices:
            # Fallback to hardcoded preset voices if API fails
            preset_voice_names = [
        ...
        # Group voice IDs into preset and custom voices
        preset_voice_names = []
        preset_voice_ids = []
        other_voice_names = []
        other_voice_ids = []
        
        # Process the voices from the API response
        for voice in voices:
            name = voice.get("name", "")
        ...
        # Return the processed voice data
        return preset_voice_names, preset_voice_ids, preset_voice_names, preset_voice_ids
        
    except Exception as e:
        print(f"Error loading voices in tts_clients.py: {e}")
